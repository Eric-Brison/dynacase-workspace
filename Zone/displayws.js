...
function redisplaywsdiv(event) {
	...
	if (isIE && (window == top))
		ww -= 18; // vertical scroll bar always
	if (isIE)
		ww += 12;
	// if (isIE) wh-=20;
	// alert(ww+' x '+wh);
	...
	if (COL2H1 && (COL2H1 > 0))
		...
		ch = wh - 250;
	if (!dtabclip)
		ch = wh - 14 - 30; // 30 is the height os searches
	if (ROW2W1 && (ROW2W1 > 0)) {
		if (ROW2W1 < 55)
			ROW2W1 = 55;
		wcol2 = ROW2W1 - parseInt(dcol1 ? dcol1.style.width : -10);
		wcol3 = ww - ROW2W1 - 30;
	} else {
		wcol2 = 198;
		wcol3 = ww - 280;
	...
	if (isIE)
		dresume.style.borderStyle = 'none';
